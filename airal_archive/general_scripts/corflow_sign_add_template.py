...
wd_tier_re_pattern = "tx.*" #regular expression to identify the word tier(s)
input_path = "input_files/"
output_path = "output_files/"
eaf_files = glob.glob(input_path+"/*.eaf")
exception_file = input_path+"dolg1241_exceptions.csv"
speaker_count = 2 # specify if there is at least one file with multiple speakers
# Default condition. Adjust if necessary.
def status_detect(arg1,arg2):
    if arg1 == None:
        if arg2.content.isupper():
            arg1 = "affix"
        else:
            arg1 = "root"
    return arg1
# Attention: There are a few places in the script further below which also need to be adjusted. Look out for #TO-BE-EDITED.
...
#TO-BE-EDITED: The following function is a subfunction used in the function 'find_tier_triples'. It ensures, that all found tiers belong to the same speaker. So far this was only used for Dolgan. For other languages, it may be necessary to adjust the function or to write another custom function.
def tier_name_condition(tier1,tier2,tier3):
...
df = pd.read_csv(exception_file,sep=";")
#Turning Null values (NA/NaN) into empty strings.
df = df.fillna("")


# for datsets with multiple speakers
if speaker_count == 1:
    sort_func = None
elif speaker_count >= 2:
    sort_func = tier_name_condition
else:
    print("Please give an integer > 0.")
...
for file in eaf_files:
    ...
    for gl_tier,mb_tier,wd_tier in find_tier_triples(trans,gl_tier_re_pattern,mb_tier_re_pattern,wd_tier_re_pattern,sort_func):
        print(f"gloss tier: {gl_tier.name}")
        print(f"morph tier: {mb_tier.name}")
        print(f"txt tier: {wd_tier.name}")
        ...
        for gl_seg in gl_tier:
            ...
            wd_seg = wd_tier.getTime(gl_seg.start)
...
#                    status = "root"
            ...
            status = status_detect(status,gl_seg)
            ...
            if wd_seg == None:
                if (gl_seg.index() - 1) >= 0:
                    ...
                    if prev_gl_seg.end == gl_seg.start:
                        wd_seg = wd_tier.getTime(prev_gl_seg.start)
                        if wd_seg == None:
                            continue
                        elif wd_seg.end == gl_seg.end:
                            root_encounter = False
            elif gl_seg.end == wd_seg.end:
                root_encounter = False
